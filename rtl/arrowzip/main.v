...
`include "builddate.v"
	...
	// Wishbone slave definitions for bus wb, slave flexpress
	wire		flexpress_sel, flexpress_ack, flexpress_stall;
	wire	[31:0]	flexpress_data;
...
//x2	Was a master bus as well
	...
	assign	   flexpress_sel = ((wb_addr[22:19] &  4'hc) ==  4'h8);
...
//x2	Was a master bus as well
	...
	begin
		casez({		console_ack,
			...
			5'b00001: wb_idata <= flexpress_data;
...
`ifdef	FLEXPRESS_ACCESS
	qflashxpress flexpressi(i_clk, i_reset,
			(wb_cyc), (wb_stb)&&(flexpress_sel),
				wb_addr[(23-3):0],
			flexpress_ack, flexpress_stall, flexpress_data,
			o_dspi_sck, o_dspi_cs_n, o_dspi_mod, o_dspi_dat, i_dspi_dat);
`else	// FLEXPRESS_ACCESS
	assign	o_dspi_sck  = 1'b0;
	assign	o_dspi_cs_n = 1'b1;
	assign	o_dspi_mod  = 2'b01;
	assign	o_dspi_dat  = 4'b1111;

	// In the case that there is no flexpress peripheral responding on the wb bus
	reg	r_flexpress_ack;
	initial	r_flexpress_ack = 1'b0;
	always @(posedge i_clk)	r_flexpress_ack <= (wb_stb)&&(flexpress_sel);
	assign	flexpress_ack   = r_flexpress_ack;
	assign	flexpress_stall = 0;
	assign	flexpress_data  = 0;

`endif	// FLEXPRESS_ACCESS
