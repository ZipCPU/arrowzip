...
`endif
	...
	// Wishbone slave definitions for bus wb(SIO), slave flash_cfg
	...
	wire		flash_sel, flash_ack, flash_stall, flash_err;
 ...
 // 0x000008
	assign	   flash_cfg_sel = ((wb_sio_sel)&&(wb_addr[ 2: 0] ==  3'h3));
 // 0x00000c
 ...
 // 0x000040 - 0x00005f
	assign	     console_sel = ((wb_addr[21:18] &  4'hf) ==  4'h1); // 0x100000 - 0x10000f
	assign	      wb_sio_sel = ((wb_addr[21:18] &  4'hf) ==  4'h2); // 0x200000 - 0x20001f
//x2	Was a master bus as well
	assign	      wb_dio_sel = ((wb_addr[21:18] &  4'hf) ==  4'h3); // 0x300000 - 0x30007f
//x2	Was a master bus as well
	assign	       bkram_sel = ((wb_addr[21:18] &  4'hf) ==  4'h4); // 0x400000 - 0x400fff
...
//x2	Was a master bus as well
	...
	always	@(posedge i_clk)
		...
		casez( wb_addr[2:0] )
			...
			3'h3: r_wb_sio_data <= flash_cfg_data;
	...
	begin
		casez({		console_ack,
				...
				wb_dio_ack,
				bkram_ack	})
			4'b1???: wb_idata <= console_data;
			4'b01??: wb_idata <= wb_sio_data;
			4'b001?: wb_idata <= wb_dio_data;
			4'b0001: wb_idata <= bkram_data;
	...
	assign wb_err = ((wb_stb)&&(wb_none_sel))||(wb_many_ack)||((flash_err));
	//
	// BUS-LOGIC for hb
	//
...
`ifdef	BKRAM_ACCESS
	memdev #(.LGMEMSZ(12), .EXTRACLOCK(1))
		bkrami(i_clk, i_reset,
			(wb_cyc), (wb_stb)&&(bkram_sel), wb_we,
				wb_addr[(12-3):0], wb_data, wb_sel,
...
`ifdef	FLASH_ACCESS
	dualflexpress flashi(i_clk, i_reset,
...
`ifdef	SPIO_ACCESS
	spio #(.NBTN(4), .NLEDS(4))) spioi(i_clk,
