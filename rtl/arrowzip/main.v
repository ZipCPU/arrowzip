...
//
	...
	wire	memsdbg_int;	// memsdbg.INT.FLASHDBG.WIRE
...
`include "builddate.v"
	...
	// Wishbone slave definitions for bus wb, slave memsdbg
	wire		memsdbg_sel, memsdbg_ack, memsdbg_stall;
	wire	[31:0]	memsdbg_data;
	...
	assign	    bustimer_sel = ((wb_dio_sel)&&((wb_addr[ 9: 8] &  2'h3) ==  2'h0));  // 0x000000
	assign	    watchdog_sel = ((wb_dio_sel)&&((wb_addr[ 9: 8] &  2'h3) ==  2'h1));  // 0x000400
	assign	         rtc_sel = ((wb_dio_sel)&&((wb_addr[ 9: 8] &  2'h3) ==  2'h2));  // 0x000800 - 0x00081f
	assign	        mems_sel = ((wb_dio_sel)&&((wb_addr[ 9: 8] &  2'h3) ==  2'h3));  // 0x000c00 - 0x000fff
	assign	    flashcfg_sel = ((wb_addr[22:19] &  4'hf) ==  4'h1); // 0x200000
	assign	     memsdbg_sel = ((wb_addr[22:19] &  4'hf) ==  4'h2); // 0x400000 - 0x400007
...
//x2	Was a master bus as well
	assign	      wb_dio_sel = ((wb_addr[22:19] &  4'hf) ==  4'h5); // 0xa00000 - 0xa00fff
//x2	Was a master bus as well
	assign	       bkram_sel = ((wb_addr[22:19] &  4'hf) ==  4'h6); // 0xc00000 - 0xc07fff
...
//x2	Was a master bus as well
	...
	casez(wb_addr[9:8])
		...
		2'b11: r_wb_dio_bus_select <= 2'd3;
...
`ifdef	MEMSSCOPE_SCOPC
	wbscopc #(.LGMEM(9),
		.SYNCHRONOUS(1))
	memsdbgi(i_clk, 1'b1, mems_debug[31], mems_debug[30:0],
		i_clk, wb_cyc, (wb_stb)&&(memsdbg_sel), wb_we,
		wb_addr[0], wb_data, memsdbg_ack, memsdbg_stall,
		memsdbg_data, memsdbg_int);
`else	// MEMSSCOPE_SCOPC
	assign	memsdbg_int = 0;

	// In the case that there is no memsdbg peripheral responding on the wb bus
	assign	memsdbg_ack   = (wb_stb) && (memsdbg_sel);
	assign	memsdbg_stall = 0;
	assign	memsdbg_data  = 0;

	assign	memsdbg_int = 1'b0;	// memsdbg.INT.FLASHDBG.WIRE
...
`else	// MEMS_ACCESS
	...
	assign	mems_ack   = (wb_stb) && (mems_sel);
	assign	mems_stall = 0;
	assign	mems_data  = 0;
