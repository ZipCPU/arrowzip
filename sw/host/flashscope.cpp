...
class	DUALFLEXPRESSCOPE : public SCOPE {
...
public:
	DUALFLEXPRESSCOPE(FPGA *fpga, unsigned addr, bool vecread)
		: SCOPE(fpga, addr, true, vecread) {};
	~DUALFLEXPRESSCOPE(void) {}
...
#ifndef	R_FLASHSCOPE
	printf(
"This design was not built with a flash scope attached to the DUALFLEXPRESS\n"
"design component.\n"
"\n"
"To use this design, create and enable a flash, and the DUALFLEXPRESS scope from\n"
...
"within the Makefile of the same directory.\n");
	...
	DUALFLEXPRESSCOPE *scope = new DUALFLEXPRESSCOPE(m_fpga, WBSCOPE, true);
	...
	if (!scope->ready()) {
		...
		scope->print();
		scope->writevcd("dualflexpress.vcd");
	}
	delete	m_fpga;
#endif
}
